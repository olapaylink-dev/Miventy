...
import { createOffer, transactionLineItems } from '../../util/api';
...
export const sendInquiry = (listing, orderData) => (dispatch, getState, sdk) => {
  ...
  //Use this listing for this transaction 
  ...
  createOffer(data,
    ...
    .then(response => {
      ...
      const processAlias = listingData?.attributes?.publicData?.transactionProcessAlias;
        if (!processAlias) {
          const error = new Error('No transaction process attached to listing');
          log.error(error, 'listing-process-missing', {
            listingId: listingData?.id?.uuid,
          });
