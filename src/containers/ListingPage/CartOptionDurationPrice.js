...
import css from './CartOptionDurationPrice.module.css';
...
const CartOptionDurationPrice = props =>{
    const intl = useIntl();
    const {selectedCatalogFolderName,catalog,images,getImageUrl,setShowCartOptions,handleEditCartItemDetails} = props;
    console.log(catalog);
        ...
        <div className={css.modal}>
            ...
            <div className={css.flex_grid}>
                {catalog.map((itm,key)=>{
                    //  const {id} = itm;
                    //  const uuid = id.uuid;
                    //  const {images={},attributes={}} = itm;
                    //  const {title,description,from,price,profileImg,publicData} = attributes;
                    //  const {category,rating="0",location} = publicData;
                    //  const img = images[0]?.attributes?.variants?.default?.url;
                    
                    const imageUrl = itm.hasOwnProperty("catalogImages") && itm.catalogImages[0]?.imgUrl

                    if(itm.folder === selectedCatalogFolderName){
                            return(
...
export default CartOptionDurationPrice;