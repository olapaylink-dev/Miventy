...
const successAction = actionType => result => ({ type: actionType, payload: result.data, lastAction:result.lastAction });
...
const initialState = {
  ...
  closeListingSuccess:false
};

export default function reducer(state = initialState, action = {}) {
  const { type, payload ,lastAction} = action;
  switch (type) {
    ...
    case UPDATE_LISTING_REQUEST:
      return { ...state, updateInProgress: true, updateListingSuccess:false, updateListingError: null,updatedListing:{} ,updateListingInProgress:false,};
    case UPDATE_LISTING_SUCCESS:
      return {
        ...
        updateInProgress: false,
        lastAction:lastAction,
        updatedListing:payload,
        updateListingSuccess:true,
        updateListingInProgress:true
        // availabilityCalendar: { ...state.availabilityCalendar },
      };
    case UPDATE_LISTING_ERROR:
      return { ...state, updateInProgress: false,updateListingSuccess:false, updateListingError: payload.response ,updatedListing:{},updateListingInProgress:false,};
    ...
    case CLOSE_LISTING_REQUEST:
      return { ...state, closeListingInProgress: true ,closeListingError:null,closeListingSuccess:false};
    case CLOSE_LISTING_SUCCESS:
      return { ...state, closeListingInProgress: false, closeListingError:null, closeListingSuccess:true };
...
export const closeListingSuccess = () => ({
  ...
  return (dispatch, getState, sdk) => {
    ...
    return sdk.ownListings
      ...
      .then(() => {
        ...
        createDraftResponse.lastAction = action;
...
export function requestUpdateListing(data,action) {
  return (dispatch, getState, sdk) => {
    ...
    // If images should be saved, create array out of the image UUIDs for the API call
    //let imageProperty = typeof images !== 'undefined' ? { images: [...imageIds(images),...existingImageIds] } : {};
    //let imageProperty = typeof images !== 'undefined' ? { images: [...imageIds(images)] } : {};
    // imageProperty = {images:[...imageProperty.images,]};
    ...
    return updateStockOfListingMaybe(id, stockUpdate, dispatch)
      ...
      .then(response => {
        ...
        //   fetchLoadDataExceptions(dispatch, listing, searchString, "");
        // }
...
export const closeListing = (listingId,state) =>(dispatch,getState,sdk)=>{
...
export function requestImageUpload(actionPayload, listingImageConfig) {
  return (dispatch, getState, sdk) => {
    ...
    const queryParams = {
      expand: true,
      'fields.image': ['variants.square-small', 'variants.square-small2x','default'],
    };
