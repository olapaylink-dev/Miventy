...
export const FETCH_TRX_MESSAGES_REQUEST = 'app/TransactionPage/FETCH_TRX_MESSAGES_REQUEST';
...
export default function transactionPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case FETCH_TRX_MESSAGES_REQUEST:
      return { ...state,messages:[], fetchMessagesInProgress: true, fetchMessagesError: null };
...
const fetchTrxMessagesRequest = () =>({type:FETCH_TRX_MESSAGES_REQUEST});
...
const fetchMessages = (txId, page, config) => (dispatch, getState, sdk) => {
  ...
  return sdk.messages
    ...
    .then(response => {
      ...
      dispatch(fetchMessagesSuccess(messages.reverse(), pagination));
...
const fetchTxMessages = (txId, page) => (dispatch, getState, sdk) => {
  const paging = { page, perPage: MESSAGES_PAGE_SIZE };
  dispatch(fetchMessagesRequest());
...
export const fetchTrxMessages = (txId, page) => (dispatch, getState, sdk) => {
  const paging = { page, perPage: MESSAGES_PAGE_SIZE };
  dispatch(fetchTrxMessagesRequest());
...
export const createProposal = (txId, offer) => (dispatch, getState, sdk) => {
  dispatch(fetchTransactionRequest());
  
  return updateTransaction({txId, data:JSON.stringify(offer)})
          .then(response=>{
            dispatch(addMarketplaceEntities(response));
            dispatch(fetchTransactionSuccess(response));
