...
const PublishSuccessView = props=>{
  const intl = useIntl();
  ...
  return (
                <div className={css.modal_complete_profile_busi}>
                  <div className={classNames(css.flex_col)}>
                    <span className={css.completed_header}>{intl.formatMessage({ id: "Dashboard.successful" })}</span>
                    ...
                    {intl.formatMessage({ id: "Dashboard.yourListingWas" })}
                    </p>
                    <button className={css.back_btn} onClick={handleBackToDashboard}>{intl.formatMessage({ id: "Dashboard.close" })}</button>
                  </div>
...
export default PublishSuccessView;